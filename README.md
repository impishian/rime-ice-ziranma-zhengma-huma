...
**雾凇拼音、搜狗词库、自定义短语辅码方案、多种对候选字词进行二次筛选的方案 + 双辅自然码 + CJK-E郑码 + CJK-I虎码**

将自己常用的输入法整合到一套 rime 配置里。记述了定制整合过程。在后面的树形 “文件列表” 部分，记述了在雾凇基础上增删改的文件。
...
双拼2，是 copy & paste 双拼而得到的一个新schema，只是 **将拆字的二次筛选方式，由汉字部件，改为自然码辅码**。
...
如果想用小鹤音形的形码、86五笔、郑码或虎码来来二次筛选，可以修改 double_pinyin2.schema.yaml 里的这行配置，@ 后面改为具体的方案。
...
约 18 万字词。自从用虎码后，郑码目前少用了。以前很喜欢用，因此，留在这儿。
...
边输入边思考，还能连续打词吗？能连续打词的，就继续打词，不急于空格推上去（能连续打，都是4键以上的）。需及时断句的，要及时把前面内容推上去，用自然虎形（虎码首末两码为辅码）打个别单字（刚好4键），... ，接着再继续思考，再接着尽量打词。 

(3) **打词不带辅**，带辅会影响打字节奏（两码一字）。 
