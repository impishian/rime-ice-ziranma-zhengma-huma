...
**第二种：以词定字**（打词后用左右中括号[或]定字）、**拆字辅码**（打难字，在双拼后，`+部件的辅码）、**拆字组字**（打难字时，uU+各部件双拼）。
...
mu`uv = 沐（解释：uv = 三点水的双拼）

mu`mojn = 幕（解释：mo = 莫，jn = 巾，通常 jn 不用输，就看到所需的字了）

mu`nqtu = 牡（解释：nq = 牛，tu = 土，通常 nqtu 也不必全输完，就看到所需的字了）

(3) 拆字组字：

uUmojn 幕（解释：mo = 莫，jn = 巾）

uUnqtu 牡（解释：nq = 牛，tu = 土）
```
