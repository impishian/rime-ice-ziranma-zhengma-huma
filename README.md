...
边输入边思考，还能连续打词吗？能连续打词的，就继续打词，不急于空格推上去（都是4键以上的）。需及时断句的，及时把前面内容推上去，接下来个别字用自然虎形（虎码首末两码为辅码）打单字（刚好4键），... ，接着再继续思考，再接着尽量打词。 打词不带辅，带辅会影响两码一字的打字节奏。 对 shi shi 这样的重码较多的词，继续输入其他词后，结合上下文也没能自动调整为正确的词的话，则用下面的二次筛选（或提早断句修正，或按ESC取消重输）。
