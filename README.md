...
这个插件的**辅码码表，与现有的拼音码表，是分离的，是独立的文件。带了自然码辅码、鹤形辅码、五笔86辅码、虎码辅码。** 想用哪个都可以。

如果想用小鹤音形的形码作为辅码，或想用86五笔作为辅码，或想用虎码作为辅码，可以修改 double_pinyin2.schema.yaml 里的这行配置，@ 后面改为具体的辅码码表。

```
- lua_filter@*aux_code@ZRM_Aux-code_4.3
# rime-lua-aux-code。几种选择：(1) 自然码辅码：ZRM_Aux-code_4.3；(2) 鹤形辅码：flypy_full；(3) 86五笔作为辅码：wubi86-code；(4) 虎码作为辅码：tiger
```
